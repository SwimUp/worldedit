...
namespace WorldEdit.WorldGen
{
    ...
    internal class TerrainManager
    {
        ...
        public int SeedPart => 83469557;

        private static float FreqMultiplier => 1f;

        public void Run(GeneratorMode mode)
        {
            switch(mode)
            {
                case GeneratorMode.Temperature:
                    {
                        SetupTemperatureOffsetNoise();

                        for (int i = 0; i < Find.WorldGrid.TilesCount; i++)
                            GenerateTemperatureFor(i);
                        break;
                    }
                case GeneratorMode.Swampiness:
                    {
                        SetupSwampinessNoise();

                        for (int i = 0; i < Find.WorldGrid.TilesCount; i++)
                            GenerateSwampinessFor(i);

                        break;
                    }
                case GeneratorMode.Rainfall:
                    {
                        SetupRainfallNoise();

                        for (int i = 0; i < Find.WorldGrid.TilesCount; i++)
                            GenerateRainfallFor(i);

                        break;
                    }
                case GeneratorMode.Hilliness:
                    {
                        SetupHillinessNoise();

                        for (int i = 0; i < Find.WorldGrid.TilesCount; i++)
                            GenerateHillinessFor(i);

                        break;
                    }
                case GeneratorMode.Elevation:
                    {
                        SetupElevationNoise();

                        for (int i = 0; i < Find.WorldGrid.TilesCount; i++)
                            GenerateElevationFor(i);

                        break;
                    }
                case GeneratorMode.Biome:
                    {
                        for (int i = 0; i < Find.WorldGrid.TilesCount; i++)
                            GenerateBiomeFor(i);
        ...
        private void GenerateTemperatureFor(int tileID)
        {
        ...
        private void GenerateSwampinessFor(int tileID)
        {
            Tile tile = Find.WorldGrid[tileID];
            Vector3 tileCenter = Find.WorldGrid.GetTileCenter(tileID);
        ...
        private void GenerateRainfallFor(int tileID)
        {
            Tile tile = Find.WorldGrid[tileID];
            Vector3 tileCenter = Find.WorldGrid.GetTileCenter(tileID);
        ...
        private void GenerateHillinessFor(int tileID)
        {
            Tile tile = Find.WorldGrid[tileID];
            Vector3 tileCenter = Find.WorldGrid.GetTileCenter(tileID);
        ...
        private void GenerateElevationFor(int tileID)
        {
            Tile tile = Find.WorldGrid[tileID];
            Vector3 tileCenter = Find.WorldGrid.GetTileCenter(tileID);
        ...
        private void GenerateBiomeFor(int tileID)
        {
            Tile tile = Find.WorldGrid[tileID];
