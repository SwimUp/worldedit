...
{
    public class Generator_AncientRoads_Vanilla : Generator
    {
        ...
        public override GeneratorMode Mode => GeneratorMode.AncientRoads;
                ...
                for (int j = 0; j < ancientSites.Count; j++)
                {
                    ...
                    while (true)
                    {
                        list3 = (from idx in list3
                                 where idx != current && Math.Abs(Find.World.grid.GetHeadingFromTo(current, idx) - ang) < maximumSiteCurve
                                 select idx).ToList();
                        if (list3.Count == 0)
                        {
                            break;
